...
void unpackReceivedData(std::vector<uint8_t>* buffer) {
    // length 用于存储当前帧的长度，而 frame 是一个向量，用于存储当前帧的数据。
    int length = 0;
    std::vector<uint8_t> frame;
    UnpackStep unpack_step = UnpackStep::HEADER_SOF;

    // 确保buffer中存在完整的一帧
    // 一直执行，直到 buffer 中的数据不足以构成一帧或者当前帧已经完整接收。
    while (buffer->size() >=2 && buffer->size() >= length) {
        switch (unpack_step) {
            ...
            case UnpackStep::HEADER_SOF:
                ...
                break;
            // 获取帧长度的阶段
            // 程序计算出整个帧的长度（包括帧头、帧尾、命令等部分），并将数据存储在 frame 向量中
...
int main(int argc, char **argv)
{
    ...
    radar_range_msg.max_range = 1.6;
    radar_range_msg.field_of_view = M_PI / 6;
