...
** Output: CRC checksum
*/
uint8_t getCRC8Checksum(uint8_t* pchMessage, uint16_t dwLength, uint8_t ucCRC8) {
  uint8_t ucIndex;
  while (dwLength--) {
...
uint8_t getCRC8Checksum(std::vector<uint8_t> pchMessage, uint16_t dwLength, uint8_t ucCRC8) {
  uint8_t ucIndex;
  std::vector<uint8_t>::iterator it = pchMessage.begin();
  while (dwLength--) {
    ucIndex = ucCRC8 ^ (*it++);
    ucCRC8  = CRC8_TABLE[ucIndex];
  }
...
uint8_t verifyCRC8Checksum(std::vector<uint8_t> pchMessage, uint16_t dwLength) {
  uint8_t ucExpected = 0;
  if ((pchMessage.empty()) || (dwLength <= 2))
    return 0;
  ucExpected = getCRC8Checksum(pchMessage, dwLength - 1, CRC8_INIT);
    return (ucExpected == pchMessage[dwLength - 1]);
}
...
void appendCRC8Checksum(std::vector<uint8_t>* pchMessage, uint16_t dwLength) {
  ...
  if ((pchMessage->empty()) || (dwLength <= 2))
    return;
  ucCRC                    = getCRC8Checksum(*pchMessage, dwLength - 1, CRC8_INIT);
  pchMessage->push_back(ucCRC);
}
...
uint16_t getCRC16Checksum(std::vector<uint8_t> pchMessage, uint32_t dwLength, uint16_t wCRC) {
  uint8_t chData;
  if (pchMessage.empty()) {
    return 0xFFFF;
  }
...
void appendCRC16Checksum(std::vector<uint8_t>*  pchMessage, uint32_t dwLength) {
  ...
  wCRC                     = getCRC16Checksum(*pchMessage, dwLength - 2, CRC16_INIT);
  pchMessage->push_back((uint8_t)(wCRC & 0x00ff));
  pchMessage->push_back((uint8_t)((wCRC >> 8) & 0x00ff));
}
