...
void turn_on_robot::Control()
{
  while(ros::ok())
  {
                                       ...
                                       //通过串口读取并校验下位机发送过来的数据，然后数据转换为国际单位
    {
      ...
      //Publish_Odom();      //Pub the speedometer topic //发布里程计话题//change to another bag imu_odometry to send odom msg,if neccesary change back
...
turn_on_robot::turn_on_robot():Sampling_Time(0),Power_voltage(0)
{
 ...
 // odom_publisher    = n.advertise<nav_msgs::Odometry>("/wheeltec_robot/odom", 50); //Create the odometer topic publisher //创建里程计话题发布者//12.11 slh the same name as bicycle_wheel_odometry.cpp
